...
def is_vpc(cihaz):
    return get_cihaz_prefix(cihaz) == "vPC"
...
def format_vpc_port(port: str, node_ismi: str, satir_numarasi: int) -> str:
    # eth0 mgmt olduğu için eth1, eth2... kullanılabilir
    normalized = port.lower()
    if not normalized.startswith('eth'):
        raise ValueError(
            f"\n!!! PORT HATASI !!! Satır {satir_numarasi}: "
            f"{node_ismi} '{port}' geçersiz. Linux/vPC cihazlarda sadece eth portları kullanılabilir. "
            f"Dogru yazim = eth1, eth2 gibi"
        )
    return normalized  # eth1, eth2 olarak döner
...
def format_port(port: str, node_ismi: str, satir_numarasi: int) -> str:
    ...
    elif get_cihaz_prefix(node_ismi) == "vPC":
        return format_vpc_port(port, node_ismi, satir_numarasi)
    else:
...
def mgmt_port_map(cihaz):
    if is_vios_router(cihaz) or is_viosl2_switch(cihaz):
        ...
        return "Gi1"
    elif get_cihaz_prefix(cihaz) == "vPC":
        return "eth0"
    else:
        return "Ethernet0/0"
...
def dosyayi_isle(input_path):
    ...
    for satir_numarasi, satir in enumerate(lines, 1):
        ...
        # IP hesapla (guncellenmis_ipler'i günceller) ve sonuçları kaydet
        cihaz1_ip, cihaz2_ip, mask, link_index = ip_hesapla(cihaz1_adi, cihaz2_adi)
        ...
        ham_links.append((cihaz1_adi, cihaz1_portu, cihaz2_adi, cihaz2_portu, link_index, cihaz1_ip, cihaz2_ip, mask))

    # ── Tüm IP'ler artık kesinleşti, ansible_data'yı şimdi doldur ──
    for cihaz1_adi, cihaz1_portu, cihaz2_adi, cihaz2_portu, link_index, cached_ip1, cached_ip2, cached_mask in ham_links:
...
guncellenmis_ipler     = {}
switch_link_sayaci     = defaultdict(int)    # anahtar: switch adı → bir switch üzerindeki bağlantı sayısı
...
def ip_hesapla(cihaz1_adi, cihaz2_adi):
    ...
    is_router1 = input_cihaz1_prefix in ("R", "vR", "cSR", "vPC")
    is_router2 = input_cihaz2_prefix in ("R", "vR", "cSR", "vPC")
    ...
    if is_router1 and is_router2:
        ...
        switch_network = f"10.100.{switch_id}"
        switch_link_sayaci[switch] += 1
        router_ip = f"{switch_network}.{switch_link_sayaci[switch]}"
        switch_ip = ""
...
def convert_txt_to_yaml():
    desktop_path = "/home/muji/CLAB/acaldirlab/deneme"
